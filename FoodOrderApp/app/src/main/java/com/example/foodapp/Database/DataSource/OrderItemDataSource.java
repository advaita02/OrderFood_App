...
public class OrderItemDataSource {
    ...
    private String[] allColumns = {MySQLiteHelper.COLUMN_ID_ORDER_ITEM,
    ...
    public OrderItems insertOrderItem(Integer idOrder, Integer idFood,int quantity) throws ParseException {
        open();
        ...
        cursor.moveToFirst();
        OrderItems newOrderItem = cursorToOrderItem(cursor);
        cursor.close();
        close();

        return newOrderItem;
    }
    private OrderItems cursorToOrderItem(Cursor cursor) throws ParseException {
    ...
    public ArrayList<OrderItems> getOrderItemsByOrderId(Integer orderId) throws ParseException {
        ...
        while (!cursor.isAfterLast()) {
            OrderItems orderItems1 = cursorToOrderItems(cursor);
...
//        return food;
//    }
    private OrderItems cursorToOrderItems(Cursor cursor) throws ParseException {
        OrderItems orderItems = new OrderItems();
        orderItems.setId(cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_ID_ORDER_ITEM)));
